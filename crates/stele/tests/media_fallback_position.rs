...
fn test_inline_box_text_fallback_is_cup_ed_to_the_box_origin() {
    ...
    // where "ab " ends.
    //
    // Four columns because `x+y` measures 2.32 em wide and the em is
    // five-sixths of the 24x48 fallback cell's height — 40 px, so
    // `ceil(2.32 * 40 / 24)` = 4. This briefly became five when the em was
    // the whole cell height; the ratio that restored the flowing-paragraph
    // behaviour restored this width with it.
    assert!(
